 ...
 */
package br.com.finalcraft.unesp.tc.maquinaturing.desenho.oldfiles;
...
/*
    ...
    public void calculateAndSetTextOne(){
        ...
        if (bidiretec){
            ...
            for (Character character : arestaOne.getPontosDeFita()){
            ...
            for (Character character : arestaTwo.getPontosDeFita()){
                stringBuilder.append( character + "|");
            }
            stringBuilder.setLength(stringBuilder.length() - 1);
        }else {
    ...
    public void invertEdge(){
        ...
        tempAresta.getPontosDeFita().clear();
    ...
    public void delete(int sourceId, int targetId){
        if (bidiretec){
            if (sourceId == this.target.getID() && targetId == this.source.getID()){
                arestaTwo.getPontosDeFita().clear();
            }else {
                invertEdge();
            }
            setBidiretec(false);
